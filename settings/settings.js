...
$("grantSitePermission")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (hasAccess) {
      ...
      if (granted) {
        // Edge case: prompt to keep access, in case the user granted by accident.
        if (!confirm("Keep always-on access for YouTube? Click Cancel to revoke immediately.")) {
          await browser.permissions.remove({ origins: YOUTUBE_ORIGINS }).catch(() => {});
        }
